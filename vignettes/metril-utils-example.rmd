...
```{r}
confusion_matrix(predictions = fake_predictions_data$est.risk.score, labels = fake_predictions_data$true.risk.bin, threshold = .5)
```
## fpr
...
```{r}
fpr(predictions = fake_predictions_data$est.risk.score, labels = fake_predictions_data$true.risk.bin, threshold = .5)
```
## fnr
...
```{r}
fnr(predictions = fake_predictions_data$est.risk.score, labels = fake_predictions_data$true.risk.bin, threshold = .5)
```
## tpr
...
```{r}
tpr(predictions = fake_predictions_data$est.risk.score, labels = fake_predictions_data$true.risk.bin, threshold = .5)
```
## tnr
...
```{r}
tnr(predictions = fake_predictions_data$est.risk.score, labels = fake_predictions_data$true.risk.bin, threshold = .5)
```
## auc_roc
...
```{r}
auc_roc(predictions = fake_predictions_data$est.risk.score, labels = fake_predictions_data$true.risk.bin)
```
## auc_pr
...
```{r}
auc_pr(predictions = fake_predictions_data$est.risk.score, labels = fake_predictions_data$true.risk.bin)
```
## precision
...
```{r}
precision(predictions = fake_predictions_data$est.risk.score, labels = fake_predictions_data$true.risk.bin, threshold = .5)
```
## recall
...
```{r}
recall(predictions = fake_predictions_data$est.risk.score, labels = fake_predictions_data$true.risk.bin, threshold = .5)
```
## accuracy
...
```{r}
accuracy(predictions = fake_predictions_data$est.risk.score, labels = fake_predictions_data$true.risk.bin, threshold = .5)
```
## threshold_finder
...
```{r}
threshold_finder(precision, predictions = fake_predictions_data$est.risk.score, labels = fake_predictions_data$true.risk.bin)
```
We will show how to find the optimal threshold to maximize recall

```{r}
threshold_finder(recall, predictions = fake_predictions_data$est.risk.score, labels = fake_predictions_data$true.risk.bin)
```
We will show how to find the optimal threshold to maximize accuracy

```{r}
threshold_finder(accuracy, predictions = fake_predictions_data$est.risk.score, labels = fake_predictions_data$true.risk.bin)
```
